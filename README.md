...
CodexHub v0.4.3 uses native Go binaries for the cloud server and desktop agent when release packages include them. The original Node.js server and agent remain as a fallback for development or source-only installs.
...
CodexHub can be packaged as a Trusted Web Activity. The release includes a direct-install APK: `codexhub-android-v0.4.3.apk`. See `docs/ANDROID_APP.md` and `android/twa-manifest.json`.
...
For Windows x64, the release also includes an unsigned portable build: `codexhub-companion-windows-x64-v0.4.3.zip`. Extract it and run `CodexHub Companion.exe`.

For Windows x64, the release also includes an installer/downloader: `codexhub-companion-installer-windows-x64-v0.4.3.exe`. It installs Companion under the current user, registers login startup, creates a Start Menu shortcut, and adds a Windows Apps & Features uninstall entry. Trusted code signing requires your own certificate; see `docs/WINDOWS_SIGNING.md`.
...
- `codexhub-server-v0.4.3.zip`
- `codexhub-windows-agent-v0.4.3.zip`
- `codexhub-linux-agent-v0.4.3.zip`
- `codexhub-macos-agent-v0.4.3.zip`
- `codexhub-go-binaries-v0.4.3.zip`
- `codexhub-android-twa-v0.4.3.zip`
- `codexhub-android-v0.4.3.apk`
- `codexhub-companion-v0.4.3.zip`
- `codexhub-companion-windows-x64-v0.4.3.zip`
- `codexhub-companion-installer-windows-x64-v0.4.3.exe`
- `codexhub-source-v0.4.3.zip`
...
CodexHub 可以打包为 TWA 安卓应用。本版本已经提供可直接安装的 APK：`codexhub-android-v0.4.3.apk`。配置在 `android/twa-manifest.json`，说明见 `docs/ANDROID_APP.md`。
...
Windows x64 可以直接下载未签名便携版：`codexhub-companion-windows-x64-v0.4.3.zip`。解压后运行 `CodexHub Companion.exe`。

Windows x64 也可以下载安装器：`codexhub-companion-installer-windows-x64-v0.4.3.exe`。它会安装到当前用户目录、写入开机启动、创建开始菜单快捷方式，并在 Windows “应用和功能”里登记卸载入口。受 Windows 信任的代码签名需要你的正式证书，说明见 `docs/WINDOWS_SIGNING.md`。
