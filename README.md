...
CodexHub v0.4.7 uses native Go binaries for the cloud server, desktop agent, and Farfield launcher when release packages include them. The original Node.js server and agent remain as a fallback for development or source-only installs.
...
- Farfield launcher: `cmd/codexhub-farfield`, packaged as `bin/codexhub-farfield-*`; agent packages include a pinned `farfield-runtime/` so login startup does not call `npx @latest`.
...
For Windows x64, the release also includes an unsigned portable build: `codexhub-companion-windows-x64-v0.4.7.zip`. Extract it and run `CodexHub Companion.exe`.

For Windows x64, the release also includes an installer/downloader: `codexhub-companion-installer-windows-x64-v0.4.7.exe`. It installs Companion under the current user, registers login startup, creates a Start Menu shortcut, and adds a Windows Apps & Features uninstall entry. Trusted code signing requires your own certificate; see `docs/WINDOWS_SIGNING.md`.
...
On Linux server installs, v0.4.7 defaults to SQLite-backed persistence:
...
- `codexhub-server-v0.4.7.zip`
- `codexhub-windows-agent-v0.4.7.zip`
- `codexhub-linux-agent-v0.4.7.zip`
...
- Farfield 启动器：`cmd/codexhub-farfield`，发布包内为 `bin/codexhub-farfield-*`；电脑端发布包会包含固定版本的 `farfield-runtime/`，开机启动不再调用 `npx @latest`。
...
Windows x64 可以直接下载未签名便携版：`codexhub-companion-windows-x64-v0.4.7.zip`。解压后运行 `CodexHub Companion.exe`。

Windows x64 也可以下载安装器：`codexhub-companion-installer-windows-x64-v0.4.7.exe`。它会安装到当前用户目录、写入开机启动、创建开始菜单快捷方式，并在 Windows “应用和功能”里登记卸载入口。受 Windows 信任的代码签名需要你的正式证书，说明见 `docs/WINDOWS_SIGNING.md`。
...
Linux 服务器安装从 v0.4.7 开始默认使用 SQLite 持久化：
